...
void Game_menu_stepcreate(){
	...
	if(WiiDash_button(640/2-250-42+75,55+6*40,60,10,1,1,(char*)"")){songarrowcount=songarrowcount-1;}
	...
	sprintf(temptext,"%d%s",songarrowcount," arrows");
	...
	if(WiiDash_button(320,55+8*40,600,10,1,1,(char*)"Accept")){
		...
		for(int a=0; a<songarrowcount; a++){
			if(rand()%(10)<6){
		...
		for(int a=0; a<songarrowcount*2; a++){
			outdata << rand()%(4) << endl;
