//TODO: remember tip about fat(?) related command said to cause problems
//      when writing large blocks of data to SD...

//TODO: pass over to understand / add unimported SM data
...
void Game_menu_stepimport(){
	...
	for(int b = 0; b < n_direntries; b++)
  {
		...
		if(charmatchend(charuppercase((char*)temptext), (char*)".SM"))
    {
      ...
      {
				if(WiiDash_button(320, 55+(a-stepmenuscroll)*40, 500, 10, 1, 1, (char*)""))
        {
					stepmenuscroll++;
        }
				apply_surface(320-15/2-1, 55+(a-stepmenuscroll)*40-16/2-1, WDarrowsimage, screen, &WDarrowsframes[0]);
				a++;
			}
      else if(a >= stepmenuscroll && a - stepmenuscroll < 7)
      {
				...
				if(WiiDash_button(320, 55+(a-stepmenuscroll+1)*40, 600, 10, 1, 1, (char*)temptext))
        {
